...
public class DeleteSuppliers extends JFrame {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    DeleteSuppliers frame = new DeleteSuppliers();
    ...
    public DeleteSuppliers() {
        ...
        JLabel lblDeleteFromSuppliers = new JLabel("DELETE FROM SUPPLIERS");
        lblDeleteFromSuppliers.setFont(new Font("Poppins", Font.BOLD, 18));
        lblDeleteFromSuppliers.setBounds(199, 13, 257, 16);
        contentPane.add(lblDeleteFromSuppliers);

        JLabel lblEnterTheSupplier = new JLabel("Enter the Supplier ID of the supplier you want to delete from the supplier list");
        ...
        btnNewButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    MongoCollection sup = db.getCollection("suppliers");
                    Document sampleDoc = new Document("supplier_id",Integer.parseInt(sid.getText()));
                    sup.deleteOne(sampleDoc);
                    JOptionPane.showMessageDialog(null, "ID: " + (sid.getText()) +" Deleted from the Supplier list!");
                    int choose=0;
